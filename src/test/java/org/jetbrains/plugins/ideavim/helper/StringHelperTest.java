 ...
 * @author vlan
 */
public class StringHelperTest extends FimTestCase {

  public void testFalseSpecialKey() {
    List<KeyStroke> expectedKeys = FimInjectorKt.getInjector().getParser().parseKeys("move '<-2");
    expectedKeys.addAll(FimInjectorKt.getInjector().getParser().parseKeys("<CR>"));
    assertEquals(expectedKeys, FimInjectorKt.getInjector().getParser().parseKeys("move '<-2<CR>"));
  }
  ...
  private KeyStroke parseKeyStroke(@NotNull String s) {
    final List<KeyStroke> actualStrokes = FimInjectorKt.getInjector().getParser().parseKeys(s);
    assertEquals(FimInjectorKt.getInjector().getParser().toKeyNotation(actualStrokes), 1, actualStrokes.size());
