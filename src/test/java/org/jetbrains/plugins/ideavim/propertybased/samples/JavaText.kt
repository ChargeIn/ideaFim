...
package org.jetbrains.plugins.ideafim.propertybased.samples

internal val javaText = """
/*
 * IdeaFim - Fim emulator for IDEs based on the IntelliJ platform
 * Copyright (C) 2003-2020 The IdeaFim authors
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 2 of the License, or
...
import com.flop.idea.fim.action.change.insert.InsertCompletedDigraphAction;
import com.flop.idea.fim.action.change.insert.InsertCompletedLiteralAction;
import com.flop.idea.fim.action.macro.ToggleRecordingAction;
import com.flop.idea.fim.command.*;
import com.flop.idea.fim.extension.FimExtensionHandler;
import com.flop.idea.fim.group.ChangeGroup;
import com.flop.idea.fim.group.RegisterGroup;
...
import com.flop.idea.fim.option.OptionsManager;
...
public class KeyHandler {
  ...
  public static void executeFimAction(@NotNull Editor editor,
  ...
  public void beforeHandleKey(@NotNull Editor editor,
    ...
    if (mode == CommandState.Mode.INSERT || mode == CommandState.Mode.REPLACE) {
      FimPlugin.getChange().beforeProcessKey(editor, context, key, plan);
    }
  }
  ...
  public void handleKey(@NotNull Editor editor,
        ...
        // to an entered command. Let's figure out which it is.
          ...
          if (node instanceof CommandNode) {
            ...
            if (editorState.getMode() == CommandState.Mode.INSERT || editorState.getMode() == CommandState.Mode.REPLACE) {
              shouldRecord &= FimPlugin.getChange().processKey(editor, context, key);
            } else if (editorState.getMode() == CommandState.Mode.SELECT) {
              shouldRecord &= FimPlugin.getChange().processKeyInSelectMode(editor, context, key);
            } else if (editorState.getMappingState().getMappingMode() == MappingMode.CMD_LINE) {
              shouldRecord &= FimPlugin.getProcess().processExKey(editor, key);
            }
    ...
    if (shouldRecord && editorState.isRecording()) {
      FimPlugin.getRegister().recordKeyStroke(key);
    }
  ...
  /**
   * See the description for {@link com.flop.idea.fim.command.DuplicableOperatorAction}
   */
  ...
  private void handleEditorReset(@NotNull Editor editor, @NotNull KeyStroke key, final @NotNull DataContext context, @NotNull CommandState editorState) {
    if (editorState.getCommandBuilder().isAtDefaultState()) {
      RegisterGroup register = FimPlugin.getRegister();
  ...
  private boolean handleKeyMapping(final @NotNull Editor editor,
    ...
    final KeyMapping mapping = FimPlugin.getKey().getKeyMapping(mappingState.getMappingMode());
  ...
  private boolean handleCompleteMappingSequence(@NotNull Editor editor,
    ...
    if (mappingInfo instanceof ToKeysMappingInfo) {
      ...
      final FimExtensionHandler extensionHandler = ((ToHandlerMappingInfo)mappingInfo).getExtensionHandler();
      ...
      if (extensionHandler.isRepeatable()) {
        FimRepeater.Extension.INSTANCE.clean();
      }

      processor.executeCommand(editor.getProject(), () -> extensionHandler.execute(editor, context),
        "Fim " + extensionHandler.getClass().getSimpleName(), null);

      if (extensionHandler.isRepeatable()) {
        FimRepeater.Extension.INSTANCE.setLastExtensionHandler(extensionHandler);
        FimRepeater.Extension.INSTANCE.setArgumentCaptured(null);
        FimRepeater.INSTANCE.setRepeatHandler(true);
      }

      if (shouldCalculateOffsets && !commandState.getCommandBuilder().hasCurrentCommandPartArgument()) {
        Map<Caret, FimSelection> offsets = new HashMap<>();

        for (Caret caret : editor.getCaretModel().getAllCarets()) {
          @Nullable Integer startOffset = startOffsets.get(caret);
          if (caret.hasSelection()) {
            final FimSelection fimSelection = FimSelection.Companion
              .create(UserDataManager.getFimSelectionStart(caret), caret.getOffset(),
                SelectionType.fromSubMode(CommandStateHelper.getSubMode(editor)), editor);
            offsets.put(caret, fimSelection);
  ...
  private void handleSelectRegister(@NotNull CommandState commandState, char chKey) {
    ...
    if (FimPlugin.getRegister().isValid(chKey)) {
  ...
  private void handleCommandNode(Editor editor,
    ...
    if (expectedArgumentType == Argument.Type.EX_STRING && action.getFlags().contains(CommandFlags.FLAG_COMPLETE_EX)) {
      ...
      */
      String text = FimPlugin.getProcess().endSearchCommand(editor);
  ...
  private void startWaitingForArgument(Editor editor,
    ...
    switch (argument) {
      case MOTION:
        if (editorState.isDotRepeatInProgress() && FimRepeater.Extension.INSTANCE.getArgumentCaptured() != null) {
          commandBuilder.completeCommandPart(FimRepeater.Extension.INSTANCE.getArgumentCaptured());
        }
      ...
      case EX_STRING:
        ...
        FimPlugin.getProcess().startSearchCommand(editor, context, commandBuilder.getCount(), key);
  ...
  private @NotNull CommandPartNode getKeyRoot(MappingMode mappingMode) {
    return FimPlugin.getKey().getKeyRoot(mappingMode);
  }
  ...
  public void fullReset(@Nullable Editor editor) {
    ...
    reset(editor);
    RegisterGroup registerGroup = FimPlugin.getRegisterIfCreated();
  ...
  static class ActionRunner implements Runnable {
    ...
    public void run() {
      ...
      if (register != null) {
        FimPlugin.getRegister().selectRegister(register);
      }

      executeFimAction(editor, cmd.getAction(), context);
      if (editorState.getMode() == CommandState.Mode.INSERT || editorState.getMode() == CommandState.Mode.REPLACE) {
        FimPlugin.getChange().processCommand(editor, cmd);
      }
      ...
      FimPlugin.getRegister().resetRegister();
