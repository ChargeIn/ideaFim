...
package org.jetbrains.plugins.ideafim.extension.commentary
...
class CommentaryExtensionTest : FimTestCase() {
  override fun setUp() {
    super.setUp()
  ...
  fun testBlockCommentSingle() {
    doTest(
      ...
      "<caret>if (condition) {\n" + "}\n",
      "/<caret>*i*/f (condition) {\n" + "}\n",
      FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE, JavaFileType.INSTANCE
    )
...
// |gc| |j|
  ...
  @FimBehaviorDiffers(description = "IntelliJ's uncomment leaves the leading whitespace")
  ...
  fun `test comment line with count`() {
    // Caret position is kept as the position *before* the commenting. This is how Fim works
    doTest(
      "4gcc",
      """
        final Int value1 = 42;
  ...
  @FimBehaviorDiffers(
    """
        ...
        //final int var value3 = 42;
  """,
    description = "Fim exits Visual mode before entering Command mode, and resets the caret to the start of the visual selection." +
  ...
  fun `test block comment falls back to line comment when not available`() {
    doTest(
      ...
      - Atlanta Braves
      """.trimIndent(),
      FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE, YAMLFileType.YML
    )
  }
}
