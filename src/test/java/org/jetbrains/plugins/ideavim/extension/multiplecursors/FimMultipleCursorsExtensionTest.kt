...
package org.jetbrains.plugins.ideafim.extension.multiplecursors
...
class FimMultipleCursorsExtensionTest : FimTestCase() {
  ...
  public ChangeLineAction() {
    super(new ChangeEditorActionHandler(true, CaretOrder.DECREASING_OFFSET) {
      ...
      public boolean execute(@Not${c}Null Editor editor,
        ...
        return FimPlugin.getChange().changeLine(editor, c, count);
      }
    });
  }
}"""
  ...
  @FimBehaviorDiffers(
    shouldBeFixed = false,
    description = "Fim does not have native support for multiple cursors, so fim-multiple-cursors fakes it and " +
      "keeps a track of selections added as part of additional cursors. It will only remove selections from these " +
      "additional cursors. IdeaFim has native support, so doesn't track if a selection is due to an additional cursor " +
      "so IdeaFim will remove arbitrary selections, while fim-multiple-cursors do not."
  )
  fun testRemoveSelectionVisualMode() {
    ...
    editor.fim.fimStateMachine.pushModes(FimStateMachine.Mode.VISUAL, FimStateMachine.SubMode.VISUAL_CHARACTER)
  ...
  fun testNextOccurrenceCaseSensitive() {
    val before = """@TestWithoutNeofim(reason = SkipNeofimReason.PLUGIN)
fun getCellType(${c}pos: VisualPosition): CellType {
    ...
    val after = """@TestWithoutNeofim(reason = SkipNeofimReason.PLUGIN)
fun getCellType(${s}pos$se: VisualPosition): CellType {
  ...
  @TestWithoutNeofim(SkipNeofimReason.PLUGIN)
  fun `test with tabs`() {
    val before = """
