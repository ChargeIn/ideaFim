...
class GuiCursorOptionTest : FimTestCase() {

  private fun getOptionValue(): String {
    return (com.flop.idea.fim.FimPlugin.getOptionService().getOptionValue(OptionScope.GLOBAL, OptionConstants.guicursorName) as FimString).value
  }

  private fun setValue(value: String) {
    return (com.flop.idea.fim.FimPlugin.getOptionService().setOptionValue(OptionScope.GLOBAL, OptionConstants.guicursorName, FimString(value)))
  }

  @Suppress("SpellCheckingInspection")
  fun `test parses default values`() {
    com.flop.idea.fim.FimPlugin.getOptionService().resetDefault(OptionScope.GLOBAL, OptionConstants.guicursorName)
  ...
  fun `test ignores set with missing colon`() {
    ...
    assertTrue(com.flop.idea.fim.FimPlugin.getOptionService().isDefault(OptionScope.GLOBAL, OptionConstants.guicursorName))
  }
  ...
  fun `test get effective values on update`() {
    ...
    assertEquals("Cursor", attributes.highlightGroup)
    com.flop.idea.fim.FimPlugin.getOptionService().appendValue(OptionScope.GLOBAL, OptionConstants.guicursorName, "n:ver75-OtherCursor", OptionConstants.guicursorName)
