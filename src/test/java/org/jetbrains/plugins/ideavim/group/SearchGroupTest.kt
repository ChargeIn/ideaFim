 ...
 * @author Alex Plate
 */
class SearchGroupTest : FimTestCase() {
  fun `test one letter`() {
    val pos = search(
      "w",
      """${c}one
  ...
  private fun setIgnoreCase() = com.flop.idea.fim.FimPlugin.getOptionService().setOption(OptionScope.GLOBAL, OptionConstants.ignorecaseName)
  private fun setSmartCase() = com.flop.idea.fim.FimPlugin.getOptionService().setOption(OptionScope.GLOBAL, OptionConstants.smartcaseName)
  ...
  private fun setHighlightSearch() = com.flop.idea.fim.FimPlugin.getOptionService().setOption(OptionScope.GLOBAL, OptionConstants.hlsearchName)
  private fun clearHighlightSearch() = com.flop.idea.fim.FimPlugin.getOptionService().unsetOption(OptionScope.GLOBAL, OptionConstants.hlsearchName)
  private fun setIncrementalSearch() = com.flop.idea.fim.FimPlugin.getOptionService().setOption(OptionScope.GLOBAL, OptionConstants.incsearchName)
  ...
  private fun search(pattern: String, input: String, expectedLocation: Int): Int {
    ...
    val searchGroup = com.flop.idea.fim.FimPlugin.getSearch()
    val ref = Ref.create(-1)
    RunnableHelper.runReadCommand(
      project,
      {
        // Does not move the caret!
        val n = searchGroup.processSearchCommand(editor.fim, pattern, myFixture.caretOffset, Direction.FORWARDS)
        ref.set(n)
      },
      null, null
    )
    ...
    // Tests with neofim
    val keys = "/$pattern<CR>"
    doTest(keys, input, input.replace(c, ""), FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
