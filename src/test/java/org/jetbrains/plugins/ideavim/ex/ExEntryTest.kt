...
import com.flop.idea.fim.ui.ex.ExDocument
...
class ExEntryTest : FimTestCase() {
  ...
  @FimBehaviorDiffers(description = "Fim reorders history even when cancelling entry")
  fun `test command history`() {
    ...
    // TODO: Fim behavior reorders the history even when cancelling history
//        typeExInput(":<Up>")
//        assertExText("set digraph")
//        typeText("<Up>")
//        assertExText("set incsearch")
  ...
  @TestWithoutNeofim(SkipNeofimReason.CMD)
...
//        assertEquals("set incsearch", exEntryPanel.text)
  ...
  fun `test toggle insert replace`() {
    val exDocument = exEntryPanel.entry.document as com.flop.idea.fim.ui.ex.ExDocument
  ...
  @FimBehaviorDiffers(description = "Moving one word right positions caret at end of previous word")
  fun `test move caret one WORD right`() {
    typeExInput(":set incsearch")
    caret.dot = 0
    typeText("<S-Right>")
    // TODO: Fim moves caret to "set| ", while we move it to "set |"
    assertExOffset(4)
  ...
  fun `test insert register`() {
    com.flop.idea.fim.FimPlugin.getRegister().setKeys('c', injector.parser.parseKeys("hello world"))
    com.flop.idea.fim.FimPlugin.getRegister().setKeys('5', injector.parser.parseKeys("greetings programs"))
  ...
  fun `test insert multi-line register`() {
    ...
    com.flop.idea.fim.FimPlugin.getRegister().setKeys('c', keys)
  ...
  fun `test cmap Ctrl`() {
    ...
    com.flop.idea.fim.FimPlugin.getRegister().setKeys('e', injector.parser.parseKeys("hello world"))
  ...
  private val exEntryPanel
    get() = com.flop.idea.fim.ui.ex.ExEntryPanel.getInstance()
