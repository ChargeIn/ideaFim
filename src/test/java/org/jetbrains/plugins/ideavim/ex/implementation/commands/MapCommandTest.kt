...
class MapCommandTest : FimTestCase() {
  ...
  @TestWithoutNeofim(SkipNeofimReason.DIFFERENT, "bad replace term codes")
...
"""
  ...
  // VIM-645 |:nmap|
  @TestWithoutNeofim(SkipNeofimReason.DIFFERENT, "Bad replace of term codes")
  fun testMapSpace() {
    configureByText("foo\n")
  ...
  // VIM-676 |:map|
  @TestWithoutNeofim(reason = SkipNeofimReason.VIM_SCRIPT)
  fun testBackspaceCharacterInFimRc() {
    configureByText("\n")
    injector.fimscriptExecutor.execute("inoremap # X\u0008#\n")
    typeText(injector.parser.parseKeys("i" + "#" + "<Esc>"))
    assertState("#\n")
    assertMode(FimStateMachine.Mode.COMMAND)
    typeText(commandToKeys("imap"))
  ...
  // VIM-679 |:map|
  @TestWithoutNeofim(SkipNeofimReason.DIFFERENT)
  fun testCancelCharacterInFimRc() {
    configureByText(
      """
  ${c}foo
  bar
    ...
    injector.fimscriptExecutor.execute("map \u0018i dd\n", true)
    typeText(injector.parser.parseKeys("i" + "#" + "<Esc>"))
    assertState(
      """
  #foo
  ...
  fun testBarCtrlVEscaped() {
    configureByText("${c}foo\n")
    injector.fimscriptExecutor.execute("imap a b \u0016|\u0016| c |\n")
    typeText(injector.parser.parseKeys("ia"))
    assertState("b || c foo\n")
  }

  // VIM-679 |:map|
  @TestWithoutNeofim(SkipNeofimReason.DIFFERENT, "bad term codes")
  fun testCtrlMCtrlLAsNewLine() {
    configureByText("${c}foo\n")
    injector.fimscriptExecutor.execute("map A :%s/foo/bar/g\r\u000C\n")
  ...
  fun testRemappingZeroStillAllowsZeroToBeUsedInCount() {
    ...
    injector.fimscriptExecutor.execute("map 0 ^")
  ...
  @TestWithoutNeofim(SkipNeofimReason.PLUG)
  fun testPlugMapping() {
    configureByText("\n")
    typeText(commandToKeys("nmap ,f <Plug>Foo"))
    typeText(commandToKeys("nmap <Plug>Foo iHello<Esc>"))
  ...
  fun `test map expr context`() {
    ...
    injector.fimscriptExecutor.execute(
      ...
      """.trimIndent(),
      editor.fim, context.fim, skipHistory = false, indicateErrors = true, null
    )
    typeText(injector.parser.parseKeys("t"))
