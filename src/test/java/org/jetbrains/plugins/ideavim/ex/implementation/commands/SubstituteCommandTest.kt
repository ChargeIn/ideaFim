 ...
 * @author Alex Plate
 */
class SubstituteCommandTest : FimOptionTestCase(OptionConstants.smartcaseName, OptionConstants.ignorecaseName) {
  @FimOptionDefaultAll
  ...
  @FimOptionTestConfiguration(FimTestOption(OptionConstants.ignorecaseName, OptionValueType.NUMBER, "1"))
  @FimOptionDefault(OptionConstants.smartcaseName)
  @TestWithoutNeofim(reason = SkipNeofimReason.OPTION)
  ...
  fun `test smartcase option`() {
    com.flop.idea.fim.FimPlugin.getOptionService().setOption(OptionScope.GLOBAL, OptionConstants.smartcaseName)
...
//  @FimBehaviorDiffers(description = "Fim supports :s[flags] but IdeaFim's command parser does not handle this." +
