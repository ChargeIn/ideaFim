...
class MarksCommandTest : FimTestCase() {
  ...
  fun `test correctly handles invalid mark location`() {
    ...
    com.flop.idea.fim.FimPlugin.getMark().setMark(myFixture.editor.fim, 'a', 100000)
    enterCommand("marks")
    assertExOutput(
      """mark line  col file/text
  ...
  fun `test list all marks in correct order`() {
    ...
    // Fim does not list the (, ), { or } marks. See :help :marks
    // Can't easily test 0-9 or " (locations of previously closed files from fim-info)
    enterCommand("marks")
    assertExOutput(
      """mark line  col file/text
