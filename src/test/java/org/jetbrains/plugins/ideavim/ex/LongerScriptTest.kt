...
// todo time limit
class LongerScriptTest : FimTestCase() {
  ...
  fun `test token helper function`() {
    ...
    """.trimIndent()
    val parsingTime = measureTimeMillis { FimscriptParser.parse(function) }
    println(parsingTime)
    injector.fimscriptExecutor.execute(function)
    typeText(commandToKeys("echo F('s[ubstitute]')"))
    assertExOutput("'s' | 'su' | 'sub' | 'subs' | 'subst' | 'substi' | 'substit' | 'substitu' | 'substitut' | 'substitute';\n")
  }

  fun `test matchit fim parsing time`() {
    val script = """
      "  matchit.fim: (global plugin) Extended "%" matching
      "  autload script of matchit plugin, see ../plugin/matchit.fim
      ...
      function matchit#Match_wrapper(word, forward, mode) range
        ...
        elseif b:match_words =~ ":"
          ...
          " Allow b:match_words = "GetFimMatchWords()" .
      ...
      fun! matchit#MultiMatch(spflag, mode)
        ...
        if skip =~ 'synID' && !(has("syntax") && exists("g:syntax_on"))
          ...
          try
            ...
            " We won't find anything, so skip searching, should keep Fim responsive.
            return {}
          endtry
        endif
        mark '
      ...
      " fim:sts=2:sw=2:et:
    """.trimIndent()
    val parsingTime = measureTimeMillis { FimscriptParser.parse(script) }
    assertEmpty(IdeafimErrorListener.testLogger)
    println(parsingTime)
  }

  fun `test abolish fim parsing time`() {
    val script = """
      " abolish.fim - Language friendly searches, substitutions, and abbreviations
      " Maintainer:   Tim Pope <http://tpo.pe/>
      " Version:      1.1
      " GetLatestFimScripts: 1545 1 :AutoInstall: abolish.fim
      ...
      if !exists("g:abolish_save_file")
        if isdirectory(expand("~/.fim"))
          let g:abolish_save_file = expand("~/.fim/after/plugin/abolish.fim")
        elseif isdirectory(expand("~/fimfiles")) || has("win32")
          let g:abolish_save_file = expand("~/fimfiles/after/plugin/abolish.fim")
        else
          let g:abolish_save_file = expand("~/.fim/after/plugin/abolish.fim")
        endif
      endif
      ...
      function! s:Complete(A,L,P)
        " Fim bug: :Abolish -<Tab> calls this function with a:A equal to 0
