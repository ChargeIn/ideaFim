...
class MotionLastScreenLineActionTest : FimTestCase() {
  ...
  fun `test move caret to last line of screen with inlays`() {
    ...
    configureByLines(50, "    I found it in a legendary land")
    addBlockInlay(com.flop.idea.fim.helper.EditorHelper.getOffset(myFixture.editor, 20, 5), true, 10)
  ...
  fun `test keep caret on screen when count is greater than visible lines plus inlays`() {
    ...
    assertEquals(35, screenHeight)
    configureByLines(100, "    I found it in a legendary land")
    addBlockInlay(com.flop.idea.fim.helper.EditorHelper.getOffset(myFixture.editor, 25, 5), true, 10)
