...
import com.flop.idea.fim.helper.StringHelper.parseKeys
 ...
 * @author vlan
 */
class MotionActionTest : FimTestCase() {
  fun testDoubleToggleVisual() {
    val contents = "one tw${c}o\n"
    doTest("vv", contents, contents, FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
  }
  ...
  fun testVisualMotionInnerBigWord() {
    ...
    doTest("viW", fileContents, fileContentsAfter, FimStateMachine.Mode.VISUAL, FimStateMachine.SubMode.VISUAL_CHARACTER)
  ...
  fun testEscapeInCommand() {
    ...
    """.trimIndent()
    doTest(listOf("f", "<Esc>", "<Esc>"), content, content, FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
  ...
  fun testEscapeInCommandAndNumber() {
    ...
    """.trimIndent()
    doTest(listOf("12", "<Esc>"), content, content, FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
    assertPluginError(false)
    val fimCommandState = myFixture.editor.fimStateMachine
    kotlin.test.assertNotNull(fimCommandState)
    assertEmpty(fimCommandState.commandBuilder.keys.toList())
  }
  ...
  fun testLeftRightMove() {
    ...
    doTest(listOf("14l", "2h"), before, after, FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
  }
  ...
  fun testUpDownMove() {
    ...
    """.trimIndent()
    doTest(listOf("2j", "k"), before, after, FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
  }
  ...
  fun testForwardToTab() {
    ...
    doTest(listOf("f<Tab>"), before, after, FimStateMachine.Mode.COMMAND, FimStateMachine.SubMode.NONE)
  }
...
"""
  ...
  // VIM-965 |[m|
  @TestWithoutNeofim(reason = SkipNeofimReason.DIFFERENT, "File type specific")
...
"""
  ...
  fun testSearchFromRegister() {
    com.flop.idea.fim.FimPlugin.getRegister().setKeys('a', injector.parser.stringToKeys("two"))
    typeTextInFile(
  ...
  // |v_>| |gv|
  @TestWithoutNeofim(SkipNeofimReason.DIFFERENT, "Complicated")
