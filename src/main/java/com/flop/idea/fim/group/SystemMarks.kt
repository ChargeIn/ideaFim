...
import com.flop.idea.fim.options.OptionScope.LOCAL
...
class SystemMarks {
  companion object {
    ...
    fun createOrGetSystemMark(ch: Char, line: Int, editor: Editor): LineBookmark? {
      if (!com.flop.idea.fim.FimPlugin.getOptionService().isSet(LOCAL(IjFimEditor(editor)), IjFimOptionService.ideamarksName, IjFimOptionService.ideamarksName)) return null
...
internal fun Project.createLineBookmark(editor: Editor, line: Int, mnemonic: Char): LineBookmark? {
  ...
  val group = bookmarksManager.defaultGroup
    ?: bookmarksManager.getGroup("IdeaFim")
    ?: bookmarksManager.addGroup("IdeaFim", true)
    ?: return null
