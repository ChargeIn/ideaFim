...
import com.flop.idea.fim.group.visual.FimVisualTimer.mode
import com.flop.idea.fim.group.visual.FimVisualTimer.singleTask
...
/**
 * Timer to control non-fim visual selection adjustment.
 *
 ...
 *   If IdeaFim had reacted to this selection immediately, its listeners were called twice: after selection set and
 *   after selection removing. After there both operations fim would get normal mode, what is inconvenient because user
 *   expects fim to stay in insert mode.
 *   Same approach is used in some accented characters on macOS and for Romaji layout in Japanese language (VIM-1725).
 *
 * Because of this IdeaFim doesn't react to selection change immediately. After selection was changed, fim waits some
 ...
 *
 * So, how does IdeaFim recognize that it should stay in insert mode?
 ...
 *   is called, IdeaFim starts visual (or select) mode.
 *
 ...
 *   no adjustment gets performed and IdeaFim stays in insert mode.
 */
object FimVisualTimer {

  var swingTimer: Timer? = null
  var mode: FimStateMachine.Mode? = null

  inline fun singleTask(currentMode: FimStateMachine.Mode, crossinline task: (initialMode: FimStateMachine.Mode?) -> Unit) {
    swingTimer?.stop()

    if (mode == null) mode = currentMode

    // Default delay - 100 ms
    val timer = Timer((com.flop.idea.fim.FimPlugin.getOptionService().getOptionValue(OptionScope.GLOBAL, "visualdelay") as FimInt).value) {
      timerAction(task)
    }
  ...
  inline fun timerAction(task: (initialMode: FimStateMachine.Mode?) -> Unit) {
    task(mode)
    swingTimer = null
    mode = null
  }
