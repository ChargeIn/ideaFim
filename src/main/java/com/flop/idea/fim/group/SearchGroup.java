...
import com.flop.idea.fim.api.FimSearchGroupBase;
...
import com.flop.idea.fim.regexp.CharacterClasses;
...
import com.flop.idea.fim.fimscript.model.functions.handlers.SubmatchFunctionHandler;
...
import static com.flop.idea.fim.helper.HelperKt.localEditors;
import static com.flop.idea.fim.helper.SearchHelperKtKt.shouldIgnoreCase;
import static com.flop.idea.fim.register.RegisterConstants.LAST_SEARCH_REGISTER;

@State(name = "FimSearchSettings", storages = {
  @Storage(value = "$APP_CONFIG$/fim_settings_local.xml", roamingType = RoamingType.DISABLED)
})
public class SearchGroup extends FimSearchGroupBase implements PersistentStateComponent<Element> {
  public SearchGroup() {
    FimPlugin.getOptionService().addListener(
        OptionConstants.hlsearchName,
        new OptionChangeListener<FimDataType>() {
        @Override
    ...
    final OptionChangeListener<FimDataType> updateHighlightsIfVisible =
      new OptionChangeListener<FimDataType>() {
        @Override
        public void processGlobalValueChange(@Nullable FimDataType oldValue) {
    ...
    FimPlugin.getOptionService().addListener(OptionConstants.ignorecaseName, updateHighlightsIfVisible, false);
    FimPlugin.getOptionService().addListener(OptionConstants.smartcaseName, updateHighlightsIfVisible, false);
  }
  ...
  private void setLastUsedPattern(@NotNull String pattern, int which_pat, boolean isNewPattern) {
    ...
    // Fim never actually sets this register, but looks it up on request
    FimPlugin.getRegister().storeTextSpecial(RegisterConstants.LAST_SEARCH_REGISTER, pattern);

    // This will remove an existing entry and add it back to the end, and is expected to do so even if the string value
    // is the same
    FimPlugin.getHistory().addEntry(HistoryConstants.SEARCH, pattern);
  }
   ...
   */
  @Override
  public int processSearchCommand(@NotNull FimEditor editor, @NotNull String command, int startOffset, @NotNull Direction dir) {
    ...
    // Fim's logic is spread over several methods (do_search -> searchit -> search_regcomp), and rather tricky to follow
    ...
    // If the pattern is missing, Fim checks RE_SEARCH:
    ...
    if (pattern == null || pattern.length() == 0) {
      ...
      if (pattern == null || pattern.length() == 0) {
        ...
        if (pattern == null || pattern.length() == 0) {
          FimPlugin.showMessage(MessageHelper.message("e_noprevre"));
          return -1;
        }
      }
    }
    ...
    return findItOffset(((IjFimEditor)editor).getEditor(), startOffset, 1, lastDir);
  }
   ...
   */
  @Override
  public int searchWord(@NotNull FimEditor editor, @NotNull FimCaret caret, int count, boolean whole, Direction dir) {
    TextRange range = SearchHelper.findWordUnderCursor(((IjFimEditor)editor).getEditor(), ((IjFimCaret)caret).getCaret());
    if (range == null) {
    ...
    final String pattern = SearchHelper.makeSearchPattern(
        EditorHelper.getText(((IjFimEditor)editor).getEditor(), range.getStartOffset(), range.getEndOffset()), whole);
    ...
    final int offset = findItOffset(((IjFimEditor)editor).getEditor(), range.getStartOffset(), count, lastDir);
   ...
   */
  @Override
  public int searchNext(@NotNull FimEditor editor, @NotNull FimCaret caret, int count) {
    return searchNextWithDirection(((IjFimEditor)editor).getEditor(), ((IjFimCaret)caret).getCaret(), count, lastDir);
  }
   ...
   */
  @Override
  public int searchPrevious(@NotNull FimEditor editor, @NotNull FimCaret caret, int count) {
    return searchNextWithDirection(((IjFimEditor)editor).getEditor(), ((IjFimCaret)caret).getCaret(), count,
                                   lastDir.reverse());
  }
  ...
  public boolean processSubstituteCommand(@NotNull FimEditor editor,
                                          @NotNull FimCaret caret,
                                          @NotNull LineRange range,
                                          @NotNull @NonNls String excmd,
                                          @NotNull @NonNls String exarg,
                                          @NotNull FimLContext parent) {
    // Explicitly exit visual mode here, so that visual mode marks don't change when we move the cursor to a match.
    List<ExException> exceptions = new ArrayList<>();
    if (CommandStateHelper.inVisualMode(((IjFimEditor) editor).getEditor())) {
      ModeHelper.exitVisualMode(((IjFimEditor) editor).getEditor());
    }
    ...
    if (excmd.charAt(0) == 's' && !cmd.isNul() && !Character.isWhitespace(
      ...
      if (CharacterClasses.isAlpha(cmd.charAt())) {
        FimPlugin.showMessage(MessageHelper.message(Msg.E146));
        return false;
      }
      /*
      ...
      if (cmd.charAt() == '\\') {
        ...
        if ("/?&".indexOf(cmd.charAt()) == -1) {
          FimPlugin.showMessage(MessageHelper.message(Msg.e_backslash));
          return false;
        }
        if (cmd.charAt() != '&') {
      ...
      if (lastReplace == null) {
        ...
        FimPlugin.showMessage(MessageHelper.message(Msg.e_nopresub));
        return false;
      }
    ...
    if (cmd.charAt() == '&') {
      ...
      do_all = FimPlugin.getOptionService().isSet(new OptionScope.LOCAL(editor), OptionConstants.gdefaultName, OptionConstants.gdefaultName);
      do_ask = false;
      do_error = true;
      do_ic = 0;
    }
    ...
    if (Character.isDigit(cmd.charAt())) {
      int i = cmd.getDigits();
      if (i <= 0 && do_error) {
        FimPlugin.showMessage(MessageHelper.message(Msg.e_zerocount));
        return false;
      }
      line1 = line2;
      line2 = EditorHelper.normalizeLine(((IjFimEditor) editor).getEditor(), line1 + i - 1);
    }
    ...
    if (!cmd.isNul() && cmd.charAt() != '"') {
      ...
      FimPlugin.showMessage(MessageHelper.message(Msg.e_trailing));
      return false;
    }
    ...
    if (!booleanregmmatch_tPair.getFirst()) {
      if (do_error) {
        FimPlugin.showMessage(MessageHelper.message(Msg.e_invcmd));
    ...
    int start = ((IjFimEditor) editor).getEditor().getDocument().getLineStartOffset(line1);
    int end = ((IjFimEditor) editor).getEditor().getDocument().getLineEndOffset(line2);
    ...
    int lcount = EditorHelper.getLineCount(((IjFimEditor) editor).getEditor());
    Expression expression = null;
    for (int lnum = line1; lnum <= line2 && !got_quit; ) {
      CharacterPosition newpos = null;
      int nmatch = sp.fim_regexec_multi(regmatch, editor, lcount, lnum, searchcol);
      if (nmatch > 0) {
        if (firstMatch) {
          FimPlugin.getMark().saveJumpLocation(editor);
          firstMatch = false;
        }


        String match = sp.fim_regsub_multi(regmatch, lnum, sub, 1, false);
        if (sub.charAt(0) == '\\' && sub.charAt(1) == '=') {
          String exprString = sub.toString().substring(2);
          expression = FimscriptParser.INSTANCE.parseExpression(exprString);
          if (expression == null) {
            exceptions.add(new ExException("E15: Invalid expression: " + exprString));
            expression = new SimpleExpression(new FimString(""));
          }
        } else if (match == null) {
          return false;
        }
        ...
        int startoff = startpos.toOffset(((IjFimEditor) editor).getEditor());
        int endoff = endpos.toOffset(((IjFimEditor) editor).getEditor());

        if (do_all || line != lastLine) {
          boolean doReplace = true;
          if (do_ask) {
            RangeHighlighter hl = SearchHighlightsHelper.addSubstitutionConfirmationHighlight(((IjFimEditor) editor).getEditor(), startoff, endoff);
            final ReplaceConfirmationChoice choice = confirmChoice(((IjFimEditor) editor).getEditor(), match, ((IjFimCaret) caret).getCaret(), startoff);
            ((IjFimEditor) editor).getEditor().getMarkupModel().removeHighlighter(hl);
            switch (choice) {
          ...
          if (doReplace) {
            SubmatchFunctionHandler.INSTANCE.setLatestMatch(((IjFimEditor) editor).getEditor().getDocument().getText(new com.intellij.openapi.util.TextRange(startoff, endoff)));
            injector.getMotion().moveCaret(editor, caret, startoff);
            if (expression != null) {
              try {
              match = expression
                .evaluate(editor, new IjExecutionContext(EditorDataContext.init(((IjFimEditor) editor).getEditor(), null)), parent)
                .toInsertableString();
              } catch (Exception e) {
            ...
            ApplicationManager.getApplication().runWriteAction(() -> ((IjFimEditor) editor).getEditor().getDocument().replaceString(startoff, endoff,
                                                                                                        finalMatch));
            lastMatch = startoff;
            int newend = startoff + match.length();
            newpos = CharacterPosition.Companion.fromOffset(((IjFimEditor) editor).getEditor(), newend);
    ...
    if (!got_quit) {
      if (lastMatch != -1) {
        injector.getMotion().moveCaret(editor, caret,
          FimPlugin.getMotion().moveCaretToLineStartSkipLeading(editor, editor.offsetToLogicalPosition(lastMatch).getLine()));
      }
      else {
        FimPlugin.showMessage(MessageHelper.message(Msg.e_patnotf2, pattern));
      }
    }
    ...
    if (!exceptions.isEmpty()) {
      FimPlugin.indicateError();
      FimPlugin.showMessage(exceptions.get(0).toString());
    }
  ...
  @Override
  public int processSearchRange(@NotNull FimEditor editor,
                                @NotNull String pattern,
                                int patternOffset,
                                int startOffset,
                                @NotNull Direction direction) {
    return processSearchRange(((IjFimEditor) editor).getEditor(), pattern, patternOffset, startOffset, direction);
  }
  ...
  public Pair<Boolean, Triple<Object, String, Object>> search_regcomp(CharPointer pat,
    ...
    if (pat == null || pat.isNul()) {
      ...
      if (pattern == null) {
        FimPlugin.showMessage(errorMessage);
    ...
    sp = new RegExp();
    regmatch.regprog = sp.fim_regcomp(pattern, 1);
  ...
  private static @NotNull ReplaceConfirmationChoice confirmChoice(@NotNull Editor editor, @NotNull String match, @NotNull Caret caret, int startoff) {
    ...
    if (ApplicationManager.getApplication().isUnitTestMode()) {
      ...
      final com.flop.idea.fim.ui.ex.ExEntryPanel exEntryPanel = ExEntryPanel.getInstanceWithoutShortcuts();
      exEntryPanel.activate(editor, EditorDataContext.init(editor, null), MessageHelper.message("replace.with.0", match), "", 1);
      MotionGroup.moveCaret(editor, caret, startoff);
      ModalEntry.INSTANCE.activate(new IjFimEditor(editor), keyStrokeProcessor);
   ...
   */
  @Override
  public @Nullable TextRange getNextSearchRange(@NotNull FimEditor editor, int count, boolean forwards) {
    editor.removeSecondaryCarets();
    TextRange current = findUnderCaret(editor);

    if (current == null || CommandStateHelper.inVisualMode(((IjFimEditor)editor).getEditor()) && atEdgeOfGnRange(current, ((IjFimEditor)editor).getEditor(), forwards)) {
  ...
  private boolean atEdgeOfGnRange(@NotNull TextRange nextRange, @NotNull Editor editor, boolean forwards) {
    ...
    if (forwards) {
      return nextRange.getEndOffset() - FimPlugin.getVisualMotion().getSelectionAdj() == currentPosition;
    }
    else {
  ...
  private @Nullable TextRange findNextSearchForGn(@NotNull FimEditor editor, int count, boolean forwards) {
    if (forwards) {
      final EnumSet<SearchOptions> searchOptions = EnumSet.of(SearchOptions.WRAP, SearchOptions.WHOLE_FILE);
      return FimInjectorKt.getInjector().getSearchHelper().findPattern(editor, getLastUsedPattern(), editor.primaryCaret().getOffset().getPoint(), count, searchOptions);
    } else {
  ...
  @Override
  @Nullable
  public TextRange findUnderCaret(@NotNull FimEditor editor) {
  ...
  @Override
  @Nullable
  public TextRange searchBackward(@NotNull FimEditor editor, int offset, int count) {
    ...
    final TextRange foundBackward = FimInjectorKt.getInjector().getSearchHelper().findPattern(editor, getLastUsedPattern(), offset, count, searchOptions);
    ...
    return FimInjectorKt.getInjector().getSearchHelper().findPattern(editor, getLastUsedPattern(), startOffset, 1, searchOptions);
  }
  ...
  private void resetShowSearchHighlight() {
    showSearchHighlight = FimPlugin.getOptionService().isSet(OptionScope.GLOBAL.INSTANCE, OptionConstants.hlsearchName, OptionConstants.hlsearchName);
  }
  ...
  public static void fileEditorManagerSelectionChangedCallback(@SuppressWarnings("unused") @NotNull FileEditorManagerEvent event) {
    FimPlugin.getSearch().updateSearchHighlights();
  }
    ...
    public void documentChanged(@NotNull DocumentEvent event) {
      for (Project project : ProjectManager.getInstance().getOpenProjects()) {
        ...
        for (Editor editor : localEditors(document, project)) {
          Collection<RangeHighlighter> hls = UserDataManager.getFimLastHighlighters(editor);
          if (hls == null) {
            continue;
          }
          ...
          FimPlugin.getSearch().highlightSearchLines(editor, startPosition.line, endPosition.line);

          if (logger.isDebugEnabled()) {
            hls = UserDataManager.getFimLastHighlighters(editor);
  ...
  private int findItOffset(@NotNull Editor editor, int startOffset, int count, Direction dir) {
    boolean wrap =  FimPlugin.getOptionService().isSet(OptionScope.GLOBAL.INSTANCE, OptionConstants.wrapscanName, OptionConstants.wrapscanName);
    ...
    if (offsetIsLineOffset) {
      ...
      res = FimPlugin.getMotion().moveCaretToLineStart(new IjFimEditor(editor), newLine);
    }
    ...
    if (ppos < lastPatternOffset.length() - 1 && lastPatternOffset.charAt(ppos) == ';') {
      ...
      res = processSearchCommand(new IjFimEditor(editor), lastPatternOffset.substring(ppos + 1), res, nextDir);
    }
    return res;
  }
  ...
  private static void addOptionalTextElement(@NotNull Element element, @NotNull String name, @Nullable String text) {
    if (text != null) {
      element.addContent(FimPlugin.getXML().setSafeXmlText(new Element(name), text));
    }
  }
  ...
  public void readData(@NotNull Element element) {
    ...
    final String fimInfo = ((FimString) FimPlugin.getOptionService().getOptionValue(OptionScope.GLOBAL.INSTANCE, OptionConstants.fiminfoName, OptionConstants.fiminfoName)).getValue();
    final boolean disableHighlight = Set.of(fimInfo.split(",")).contains("h");
  ...
  private static @Nullable String getSafeChildText(@NotNull Element element, @NotNull String name) {
    ...
    return child != null ? FimPlugin.getXML().getSafeXmlText(child) : null;
  }
  ...
  private static @NotNull String getSafeChildText(@NotNull Element element, @NotNull String name, @NotNull String defaultValue) {
    final Element child = element.getChild(name);
    if (child != null) {
      final String value = FimPlugin.getXML().getSafeXmlText(child);
  ...
  // Fim saves the patterns used for searching (`/`) and substitution (`:s`) separately
  // fiminfo records them as `# Last Search Pattern` and `# Last Substitute Search Pattern` respectively
  // Fim also saves flags in fiminfo - ~<magic><smartcase><line><end><off>[~]
  // The trailing tilde tracks which was the last used pattern, but line/end/off is only used for search, not substitution
  // Search values can contain new lines, etc. Fim saves these as CTRL chars, e.g. ^M
  // Before saving, Fim reads existing fiminfo, merges and writes
  ...
  private @Nullable String lastReplace;            // `# Last Substitute String` from fiminfo
  ...
  private boolean showSearchHighlight = FimPlugin.getOptionService().isSet(OptionScope.GLOBAL.INSTANCE, OptionConstants.hlsearchName, OptionConstants.hlsearchName);
