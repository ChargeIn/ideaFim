...
import static com.flop.idea.fim.helper.SearchHelperKtKt.checkInString;
...
public class SearchHelper {
    ...
    do  /* loop for count */ {
      ...
      for (loop = 0; loop <= 1; ++loop)   /* loop twice if 'wrapscan' set */ {
        ...
        for (; lnum >= startLine && lnum < endLine; lnum += dir.toInt(), at_first_line = false) {
           ...
           */
          nmatched = sp.fim_regexec_multi(regmatch, new IjFimEditor(editor), lcount, lnum, 0);
          if (nmatched > 0) {
            ...
            if (dir == Direction.FORWARDS && at_first_line) {
              ...
              while (matchpos.lnum == 0
                ...
                if (ptr.charAt(matchcol) == '\u0000' ||
                  (nmatched = sp.fim_regexec_multi(regmatch, new IjFimEditor(editor), lcount, lnum, matchcol)) == 0) {
                  match_ok = false;
                  break;
                }
                matchpos = new RegExp.lpos_T(regmatch.startpos[0]);
            ...
            if (dir == Direction.BACKWARDS) {
              ...
              for (;;) {
                ...
                if (ptr.charAt(matchcol) == '\u0000' ||
                  (nmatched = sp.fim_regexec_multi(regmatch, new IjFimEditor(editor), lcount, lnum + matchpos.lnum, matchcol)) == 0) {
                  break;
                }
    ...
    if (found == 0)             /* did not find it */ {
      ...
      if (searchOptions.contains(SearchOptions.SHOW_MESSAGES)) {
        if (searchOptions.contains(SearchOptions.WRAP)) {
          ...
          FimPlugin.showMessage(MessageHelper.message(Msg.E384, pattern));
        }
        else {
          FimPlugin.showMessage(MessageHelper.message(Msg.E385, pattern));
        }
      }
      return null;
    }
  ...
  public static @NotNull List<TextRange> findAll(@NotNull Editor editor,
    ...
    for (int line = startLine; line <= actualEndLine; ) {
      int matchedLines = regExp.fim_regexec_multi(regMatch, new IjFimEditor(editor), lineCount, line, col);
      if (matchedLines > 0) {
  ...
  public static @Nullable TextRange findBlockTagRange(@NotNull Editor editor,
    ...
    while (true) {
      ...
      final FimStateMachine.Mode mode = FimStateMachine.getInstance(new IjFimEditor(editor)).getMode();
      if (mode == FimStateMachine.Mode.VISUAL) {
  ...
  public static @NotNull CountPosition countWords(@NotNull CharSequence chars, int start, int end, int offset) {
    ...
    int res = start;
    while (true) {
      res = (int)FimSearchHelperBase.Companion.findNextWordOne(chars, res, end, 1, true, false);
  ...
  public static @NotNull TextRange findWordUnderCursor(@NotNull Editor editor,
    ...
    if ((!onWordStart && !(startSpace && isOuter)) || hasSelection || (count > 1 && dir == -1)) {
      if (dir == 1) {
        start = (int)FimSearchHelperBase.Companion.findNextWord(chars, pos, max, -1, isBig, !isOuter);
      }
      else {
        start = (int)FimSearchHelperBase.Companion.findNextWord(chars, pos, max, -(count - (onWordStart && !hasSelection ? 1 : 0)), isBig, !isOuter);
      }
  ...
  private static int findNextWordEndOne(@NotNull CharSequence chars,
    ...
    if (step > 0 && pos < size - 1) {
      if (CharacterHelper.charType(chars.charAt(pos + 1), bigWord) == CharacterHelper.CharacterType.WHITESPACE &&
          !spaceWords) {
        pos = (int)(FimSearchHelperBase.Companion.skipSpace(chars, pos + 1, step, size) - 1);
      }
      if (pos < size - 1 &&
    ...
    while (pos >= 0 && pos < size && !found) {
      ...
      if (newType != type) {
        if (step >= 0) {
          ...
          pos = (int)FimSearchHelperBase.Companion.skipSpace(chars, pos, step, size);
          res = pos;
        }
        else {
          res = pos;
  ...
  private static @NotNull String parseMatchPairsOption() {
    String[] vals = ((FimString) FimPlugin.getOptionService()
