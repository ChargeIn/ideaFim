...
import com.flop.idea.fim.RegisterActions
...
@Service
class IjActionExecutor : FimActionExecutor {
  ...
  override fun executeFimAction(
    ...
    CommandProcessor.getInstance()
      .executeCommand(
        editor.ij.project,
        { cmd.execute(editor, EditorDataContext.init(editor.ij, context.ij).fim, operatorArguments) },
        cmd.id, DocCommandGroupId.noneGroupId(editor.ij.document), UndoConfirmationPolicy.DEFAULT,
        editor.ij.document
      )
  }

  override fun findFimAction(id: String): EditorActionHandlerBase? {
    return com.flop.idea.fim.RegisterActions.findAction(id)
  }

  override fun findFimActionOrDie(id: String): EditorActionHandlerBase {
    return com.flop.idea.fim.RegisterActions.findActionOrDie(id)
  }
