...
public class EditorHelper {
   ...
   */
  public static int getVisualLineCount(final @NotNull FimEditor editor) {
    int count = editor.lineCount();
    return count == 0
           ? 0
           : FimInjectorKt.getInjector().getEngineEditorHelper().logicalLineToVisualLine(editor, count - 1) + 1;
  }
  ...
  public static int getVisualLineCount(final @NotNull Editor editor) {
    return getVisualLineCount(new IjFimEditor(editor));
  }
  ...
  public static int normalizeVisualLine(final @NotNull Editor editor, final int line) {
    return Math.max(0, Math.min(line, getVisualLineCount(new IjFimEditor(editor)) - 1));
  }
   ...
   * number of lines and mimics what Fim does.</p>
   *
   * @param editor     The editor to scroll
  ...
  public static void scrollColumnToMiddleOfScreen(@NotNull Editor editor, int visualLine, int visualColumn) {
    ...
    // still a good solution. Besides, what kind of monster uses Fim with proportional fonts?
  ...
  public static int prepareLastColumn(@NotNull Caret caret) {
    // In most cases fimLastColumn contains a correct value. But it would be incorrect if IJ will move the caret
    //   and IdeaFim won't catch that. Here we try to detect and process this case.

    int fimLastColumn = UserDataManager.getFimLastColumn(caret);
    VisualPosition visualPosition = caret.getVisualPosition();

    // Current column equals to fimLastColumn. It's great, everything is okay.
    int inlayAwareOffset = InlayHelperKt.toInlayAwareOffset(visualPosition, caret);
    if (inlayAwareOffset == fimLastColumn) return fimLastColumn;
    ...
    // Current column is somewhere at the end and fimLastColumn is greater than last column. This might be because
    //  the previous vertical motion was from a longer line. In this case we just return fimLastColumn. But it
    //  also might be the case decribed above: IJ did move the caret and IdeaFim didn't catch that. We don't process
    //  this case and just return fimLastColumn with the hope that this won't be a big pain for the user.
    // This logic can be polished in the future.
    if ((lastColumn == visualPosition.column || lastColumn + 1 == visualPosition.column) &&
        fimLastColumn > visualPosition.column) {
      return fimLastColumn;
    }

    // Okay here we know that something is definitely wrong. We set fimLastColumn to the current column.
    int updatedCol = InlayHelperKt.getInlayAwareVisualColumn(caret);
    UserDataManager.setFimLastColumn(caret, updatedCol);
    return updatedCol;
  }

  public static void updateLastColumn(@NotNull Caret caret, int prevLastColumn) {
    UserDataManager.setFimLastColumn(caret, prevLastColumn);
  }
  ...
  public static Pair<Boolean, Integer> scrollFullPageDown(final @NotNull Editor editor, int pages) {
    final Rectangle visibleArea = getVisibleArea(editor);
    final int lastVisualLine = getVisualLineCount(new IjFimEditor(editor)) - 1;
    ...
    for (int i = 0; i < pages; i++) {
      ...
      if (targetTopVisualLine >= lastVisualLine) {
        ...
        if (i == pages - 1) {
          ...
          caretVisualLine = getVisualLineCount(new IjFimEditor(editor)) - 1;
          completed = false;
        }
  ...
  public static Pair<Boolean, Integer> scrollFullPageUp(final @NotNull Editor editor, int pages) {
    ...
    for (int i = 0; i < pages; i++) {
      ...
      if (targetBottomVisualLine == 1) {
        ...
        // Fim normally scrolls up window height minus two. When there are only one or two lines in the screen, due to
