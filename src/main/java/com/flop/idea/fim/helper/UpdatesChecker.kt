...
object UpdatesChecker {
  ...
  private const val IDEAVIM_STATISTICS_TIMESTAMP_KEY = "ideafim.statistics.timestamp"
  private val DAY_IN_MILLIS = TimeUnit.DAYS.toMillis(1)

  /**
   * Check if we have plugin updates for IdeaFim.
   *
  ...
  fun check() {
    ...
    if (!outOfDate || !com.flop.idea.fim.FimPlugin.isEnabled()) return

    ApplicationManager.getApplication().executeOnPooledThread {
      try {
        val buildNumber = ApplicationInfo.getInstance().build.asString()
        val version = URLEncoder.encode(com.flop.idea.fim.FimPlugin.getVersion(), CharsetToolkit.UTF8)
        val os = URLEncoder.encode("${SystemInfo.OS_NAME} ${SystemInfo.OS_VERSION}", CharsetToolkit.UTF8)
        val uid = PermanentInstallationID.get()

        val url = "https://plugins.jetbrains.com/plugins/list?" +
          "pluginId=${com.flop.idea.fim.FimPlugin.getPluginId().idString}" +
          "&build=$buildNumber" +
        ...
        HttpRequests.request(url).connect { request: HttpRequests.Request ->
          logger.info("Check IdeaFim updates: $url")
          try {
