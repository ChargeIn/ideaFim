...
fun updateIncsearchHighlights(
  ...
  val searchStartOffset =
    if (searchRange != null) com.flop.idea.fim.helper.EditorHelper.getLineStartOffset(editor, searchRange.startLine) else caretOffset
  val showHighlights = com.flop.idea.fim.FimPlugin.getOptionService().isSet(OptionScope.LOCAL(IjFimEditor(editor)), OptionConstants.hlsearchName)
  ...
  for (project in projectManager.openProjects) {
    ...
    for (editor in editors) {
      ...
      if (shouldAddAllSearchHighlights(editor, pattern, showHighlights)) {
        ...
        val results =
          com.flop.idea.fim.helper.SearchHelper.findAll(editor, pattern, startLine, endLine, shouldIgnoreCase(pattern, shouldIgnoreSmartCase))
        if (results.isNotEmpty()) {
          currentMatchOffset = findClosestMatch(editor, results, initialOffset, forwards)
          highlightSearchResults(editor, pattern, results, currentMatchOffset)
        }
        editor.fimLastSearch = pattern
        ...
        if (com.flop.idea.fim.FimPlugin.getOptionService().isSet(OptionScope.GLOBAL, OptionConstants.wrapscanName)) {
        ...
        val result = com.flop.idea.fim.helper.SearchHelper.findPattern(editor, pattern, initialOffset, 1, searchOptions)
        if (result != null) {
        ...
        val offset = editor.fimIncsearchCurrentMatchOffset
        if (offset != null) {
...
private fun shouldRemoveSearchHighlights(editor: Editor, newPattern: String?, hlSearch: Boolean): Boolean {
  return !hlSearch || newPattern != null && newPattern != editor.fimLastSearch
}

private fun removeSearchHighlights(editor: Editor) {
  editor.fimLastSearch = null
  val ehl = editor.fimLastHighlighters ?: return
  for (rh in ehl) {
...
private fun shouldAddAllSearchHighlights(editor: Editor, newPattern: String?, hlSearch: Boolean): Boolean {
  return hlSearch && newPattern != null && newPattern != editor.fimLastSearch && newPattern != ""
}
...
fun highlightSearchResults(editor: Editor, pattern: String, results: List<TextRange>, currentMatchOffset: Int) {
  var highlighters = editor.fimLastHighlighters
  if (highlighters == null) {
    highlighters = mutableListOf()
    editor.fimLastHighlighters = highlighters
  }
  ...
  editor.fimIncsearchCurrentMatchOffset = currentMatchOffset
}
