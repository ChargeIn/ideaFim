...
internal class FimscriptState : ApplicationUsagesCollector() {
  ...
  override fun getMetrics(): Set<MetricEvent> {
    return setOf(
      VIMSCRIPT.metric(
        ...
        IDEAVIMRC_SIZE with (FimRcService.findIdeaFimRc()?.readLines()?.filter { !it.matches(Regex("\\s*\".*")) && it.isNotBlank() }?.size ?: -1),
  ...
  companion object {
    private val GROUP = EventLogGroup("fim.fimscript", 1)
    ...
    private val IDEAVIMRC_SIZE = EventFields.RoundedInt("ideafimrc_size")
