...
class HistoryUpAction : TextAction(ExEditorKit.HistoryUp) {
  override fun actionPerformed(actionEvent: ActionEvent) {
    val target = getTextComponent(actionEvent) as com.flop.idea.fim.ui.ex.ExTextField
...
class InsertRegisterAction : TextAction(ExEditorKit.InsertRegister), MultiStepAction {
  ...
  override fun actionPerformed(e: ActionEvent) {
    val target = getTextComponent(e) as com.flop.idea.fim.ui.ex.ExTextField
      ...
      when (state) {
        ...
        State.WAIT_REGISTER -> {
          ...
          if (c != KeyEvent.CHAR_UNDEFINED) {
            val register = com.flop.idea.fim.FimPlugin.getRegister().getRegister(c)
            if (register != null) {
...
class CompleteEntryAction : TextAction(ExEditorKit.CompleteEntry) {
  override fun actionPerformed(actionEvent: ActionEvent) {
    ...
    val entry = com.flop.idea.fim.ui.ex.ExEntryPanel.getInstance().entry
    KeyHandler.getInstance().handleKey(entry.editor.fim, stroke, entry.context.fim)
  }
...
class DeletePreviousWordAction : TextAction(DefaultEditorKit.deletePrevWordAction) {
  ...
  override fun actionPerformed(e: ActionEvent) {
    ...
    val doc = target.document
    val caret = target.caret
    val offset = FimSearchHelperBase.Companion.findNextWord(
