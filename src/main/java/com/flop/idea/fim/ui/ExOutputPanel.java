...
public class ExOutputPanel extends JPanel {
  ...
  public static boolean isPanelActive(@NotNull Editor editor) {
    return UserDataManager.getFimMorePanel(editor) != null;
  }

  public static @NotNull ExOutputPanel getInstance(@NotNull Editor editor) {
    ExOutputPanel panel = UserDataManager.getFimMorePanel(editor);
    if (panel == null) {
      panel = new ExOutputPanel(editor);
      UserDataManager.setFimMorePanel(editor, panel);
    }
    return panel;
  }
  ...
  private void positionPanel() {
    ...
    if (!FimPlugin.getOptionService().isSet(OptionScope.GLOBAL.INSTANCE, OptionConstants.moreName, OptionConstants.moreName)) {
      // FIX
      scrollOffset(100000);
    }
    else {
  ...
  private void close(final @Nullable KeyEvent e) {
    ApplicationManager.getApplication().invokeLater(() -> {
      ...
      if (project != null && e != null && e.getKeyChar() != '\n') {
        ...
        FimPlugin.getMacro().playbackKeys(new IjFimEditor(myEditor), new IjExecutionContext(
                                              EditorDataContext.init(myEditor, null)),
                                          0, 1);
      }
    });
  }
