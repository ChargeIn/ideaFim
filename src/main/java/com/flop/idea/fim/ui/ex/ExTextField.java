...
import com.flop.idea.fim.options.helpers.GuiCursorAttributes;
...
public class ExTextField extends JTextField {
  ...
  ExTextField() {
    ...
    addMouseListener(new MouseAdapter() {
      ...
      public void mouseClicked(MouseEvent e) {
        ...
        // the mouse is clicked anywhere. Fim's behavior is to use the mouse click as an event, which can lead to
  ...
  void setType(@NotNull String type) {
    ...
    if (hkey != null) {
      history = FimPlugin.getHistory().getEntries(hkey, 0, 0);
  ...
  void cancel() {
    clearCurrentAction();
    FimPlugin.getProcess().cancelExEntry(new IjFimEditor(EditorHolderService.getInstance().getEditor()), true);
  }
   ...
   * depending on the insert/overwrite status of the text field. This mimics Fim's behaviour.
   *
