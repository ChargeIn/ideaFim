...
import com.flop.idea.fim.helper.runAfterGotFocus
...
import com.flop.idea.fim.key.addLeafs
 ...
 */
class NerdTree : com.flop.idea.fim.extension.FimExtension {
  override fun getName(): String = pluginName

  override fun init() {
    LOG.info("IdeaFim: Initializing NERDTree extension. Disable this extension if you observe a strange behaviour of the project tree. E.g. moving down on 'j'")
    ...
    override fun actionPerformed(e: AnActionEvent) {
      ...
      when (nextNode) {
        ...
        is CommandNode<NerdAction> -> {
          ...
          when (action) {
            is NerdAction.ToIj -> callAction(action.name, e.dataContext.fim)
  ...
  private fun registerCommands() {
    ...
    registerCommand(
      ...
      NerdAction.Code { project, context, event ->
        ...
        callAction("ActivateProjectToolWindow", context.fim)
      }
    )
    registerCommand(
  ...
  companion object {
    ...
    fun callAction(name: String, context: ExecutionContext) {
      val action = ActionManager.getInstance().getAction(name) ?: run {
        com.flop.idea.fim.FimPlugin.showMessage(MessageHelper.message("action.not.found.0", name))
        return
      }
      val application = ApplicationManager.getApplication()
      if (application.isUnitTestMode) {
        injector.actionExecutor.executeAction(action.fim, context)
      } else {
        runAfterGotFocus {
    ...
    private fun addCommand(alias: String, handler: CommandAliasHandler) {
      com.flop.idea.fim.FimPlugin.getCommand().setAlias(alias, CommandAlias.Call(0, -1, alias, handler))
    }

    private fun registerCommand(variable: String, default: String, action: NerdAction) {
      val variableValue = com.flop.idea.fim.FimPlugin.getVariableService().getGlobalVariableValue(variable)
      val mappings = if (variableValue is FimString) {
        variableValue.value
      } else {
        default
      }
    ...
    private fun installDispatcher(project: Project) {
      ...
      dispatcher.registerCustomShortcutSet(
        com.flop.idea.fim.group.KeyGroup.toShortcutSet(shortcuts),
