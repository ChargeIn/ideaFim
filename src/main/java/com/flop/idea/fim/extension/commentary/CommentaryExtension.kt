 ...
 */
package com.flop.idea.fim.extension.commentary
...
import com.flop.idea.fim.extension.FimExtensionFacade.addCommand
...
class CommentaryExtension : com.flop.idea.fim.extension.FimExtension {
  ...
  companion object {
    fun doCommentary(
      ...
      val mode = editor.fimStateMachine.mode
    ...
    private fun afterCommenting(
      ...
      // This is what Fim does. If IntelliJ is configured to add comments at the start of the line, this might put
  ...
  override fun init() {
    ...
    // Previous versions of IdeaFim used different mappings to Fim's Commentary. Make sure everything works if someone
  ...
  private class CommentaryOperatorHandler : OperatorFunction, ExtensionHandler {
    ...
    override fun apply(editor: FimEditor, context: ExecutionContext, selectionType: SelectionType): Boolean {
      val range = com.flop.idea.fim.FimPlugin.getMark().getChangeMarks(editor) ?: return false
  ...
  private class CommentaryTextObjectMotionHandler : TextObjectActionHandler(), ExtensionHandler {
    ...
    override fun getRange(
      ...
      val nativeEditor = (editor as IjFimEditor).editor
      val file = com.flop.idea.fim.helper.PsiHelper.getFile(nativeEditor) ?: return null
      ...
      if (startLine <= endLine) {
        val startOffset = com.flop.idea.fim.helper.EditorHelper.getLineStartOffset(nativeEditor, startLine)
        val endOffset = com.flop.idea.fim.helper.EditorHelper.getLineStartOffset(nativeEditor, endLine + 1)
    ...
    private fun isCommentLine(file: PsiFile, editor: Editor, logicalLine: Int): Boolean {
      val startOffset = com.flop.idea.fim.helper.EditorHelper.getLineStartOffset(editor, logicalLine)
      val endOffset = com.flop.idea.fim.helper.EditorHelper.getLineEndOffset(editor, logicalLine, true)
  ...
  private class CommentaryCommandAliasHandler : CommandAliasHandler {
    override fun execute(command: String, ranges: Ranges, editor: FimEditor, context: ExecutionContext) {
