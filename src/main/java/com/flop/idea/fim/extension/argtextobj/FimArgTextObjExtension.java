...
package com.flop.idea.fim.extension.argtextobj;
...
public class FimArgTextObjExtension implements FimExtension {
  ...
  public void init() {
    ...
    putExtensionHandlerMapping(MappingMode.XO, FimInjectorKt.getInjector().getParser().parseKeys("<Plug>InnerArgument"), getOwner(), new FimArgTextObjExtension.ArgumentHandler(true), false);
    putExtensionHandlerMapping(MappingMode.XO, FimInjectorKt.getInjector().getParser().parseKeys("<Plug>OuterArgument"), getOwner(), new FimArgTextObjExtension.ArgumentHandler(false), false);

    putKeyMapping(MappingMode.XO, FimInjectorKt.getInjector().getParser().parseKeys("ia"), getOwner(), FimInjectorKt.getInjector().getParser().parseKeys("<Plug>InnerArgument"), true);
    putKeyMapping(MappingMode.XO, FimInjectorKt.getInjector().getParser().parseKeys("aa"), getOwner(), FimInjectorKt.getInjector().getParser().parseKeys("<Plug>OuterArgument"), true);
  ...
  private static String bracketPairsVariable() {
    final Object value = FimPlugin.getVariableService().getGlobalVariableValue("argtextobj_pairs");
    if (value instanceof FimString fimValue) {
      return fimValue.getValue();
    }
    return null;
  }
  ...
  static class ArgumentHandler implements ExtensionHandler {
      ...
      public TextRange getRange(@NotNull FimEditor editor,
        ...
        if (bracketPairsVar != null) {
          try {
            ...
            @FimNlsSafe String message =
              MessageHelper.message("argtextobj.invalid.value.of.g.argtextobj.pairs.0", parseException.getMessage());
            FimPlugin.showMessage(message);
            FimPlugin.indicateError();
            return null;
          }
        }
        final ArgBoundsFinder finder = new ArgBoundsFinder(((IjFimEditor)editor).getEditor().getDocument(), bracketPairs);
        int pos = ((IjFimCaret)caret).getCaret().getOffset();

        for (int i = 0; i < count; ++i) {
          if (!finder.findBoundsAt(pos)) {
            FimPlugin.showMessage(finder.errorMessage());
            FimPlugin.indicateError();
            return null;
          }
          if (i + 1 < count) {
    ...
    public void execute(@NotNull FimEditor editor, @NotNull ExecutionContext context) {
      ...
      if (!fimStateMachine.isOperatorPending()) {
        fimEditor.getEditor().getCaretModel().runForEachCaret((Caret caret) -> {
  ...
  private static class ArgBoundsFinder {
    ...
    private void getOutOfQuotedText() {
      // TODO this method should use IdeaFim methods to determine if the current position is in the string
