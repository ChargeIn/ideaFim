...
import com.flop.idea.fim.action.motion.select.SelectToggleVisualMode
  ...
  class ActionListener : AnActionListener {
    ...
    override fun afterActionPerformed(action: AnAction, event: AnActionEvent, result: AnActionResult) {
      ...
      if (ActionManager.getInstance().getId(action) == IdeActions.ACTION_CHOOSE_LOOKUP_ITEM) {
        val myEditor = editor
        if (myEditor != null) {
          FimVisualTimer.doNow()
          if (myEditor.inVisualMode) {
            SelectToggleVisualMode.toggleMode(myEditor.fim)
            KeyHandler.getInstance().partialReset(myEditor.fim)
          }
        }
      }
    }
