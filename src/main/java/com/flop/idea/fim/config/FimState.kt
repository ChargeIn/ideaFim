...
package com.flop.idea.fim.config
...
private class StateProperty(val xmlName: String) : ReadWriteProperty<FimState, Boolean> {

  init {
    map[xmlName] = false
  }

  override fun getValue(thisRef: FimState, property: KProperty<*>): Boolean = map.getOrPut(xmlName) { false }

  override fun setValue(thisRef: FimState, property: KProperty<*>, value: Boolean) {
    map[xmlName] = value
  }
}
