...
abstract class FimStringParserBase : FimStringParser {
  ...
  // todo what is the difference between this one and com.flop.idea.fim.helper.EngineStringHelper#toPrintableCharacters
  ...
  override fun toKeyNotation(keyStroke: KeyStroke): String {
    ...
    var name = getFimKeyValue(keyCode)
    if (name != null) {
  ...
  private fun parseMapLeader(s: String): List<KeyStroke>? {
    if ("leader".equals(s, ignoreCase = true)) {
      ...
      return if (mapLeader is FimString) {
...
//  }.toSet()
  ...
  private fun getFimKeyValue(c: Int): @NonNls String? {
    return when (c) {
  ...
  override fun parseFimScriptString(string: String): String {
    val result = StringBuilder()
    var state = FimStringState.INIT
    var specialKeyBuilder: StringBuilder? = null
    var digitsLeft = 0
    var number = 0
    val fimStringWithForceEnd = string + 0.toChar()
    var i = 0
    while (i < fimStringWithForceEnd.length) {
      val c = fimStringWithForceEnd[i]
      when (state) {
        FimStringState.INIT -> if (c == '\\') {
          state = FimStringState.ESCAPE
        } else if (c.code == 0) {
          i = fimStringWithForceEnd.length
        } else {
          result.append(c)
        }

        FimStringState.ESCAPE -> {
          val octalToDigital = octalDigitToNumber(c)
          if (octalToDigital != null) {
            number = octalToDigital
            digitsLeft = 2
            state = FimStringState.OCTAL_NUMBER
          } else if (Character.toLowerCase(c) == 'x') {
            digitsLeft = 2
            state = FimStringState.HEX_NUMBER
          } else if (c == 'u') {
            digitsLeft = 4
            state = FimStringState.HEX_NUMBER
          } else if (c == 'U') {
            ...
            state = FimStringState.SPECIAL
            specialKeyBuilder = StringBuilder()
          } else if (c.code == 0) {
            i = fimStringWithForceEnd.length // force end of the string
          } else {
            result.append(c)
            state = FimStringState.INIT
          }
        ...
        FimStringState.OCTAL_NUMBER -> {
          val value = octalDigitToNumber(c)
          if (value != null) {
            digitsLeft -= 1
            number = number * 8 + value
            if (digitsLeft == 0 || i == fimStringWithForceEnd.length - 1) {
              if (number != 0) {
  ...
  private enum class FimStringState {
  ...
  private fun parseSpecialKey(s: String, modifiers: Int): KeyStroke? {
    ...
    val keyCode = getFimKeyName(lower)
    val typedChar = getFimTypedKeyName(lower)
    if (keyCode != null) {
  ...
  private fun getFimKeyName(lower: @NonNls String?): Int? {
    return when (lower) {
  ...
  private fun getFimTypedKeyName(lower: String): Char? {
    return when (lower) {
      "space" -> ' '
      "bar" -> '|'
      "bslash" -> '\\'
