...
class CommandBuilder(private var currentCommandPartNode: CommandPartNode<FimActionsInitiator>) {
  ...
  fun addCountCharacter(key: KeyStroke) {
    ...
    // If count overflows and flips negative, reset to 999999999L. In Fim, count is a long, which is *usually* 32 bits,
  ...
  fun setCurrentCommandPartNode(newNode: CommandPartNode<FimActionsInitiator>) {
    currentCommandPartNode = newNode
  }

  fun getChildNode(key: KeyStroke): Node<FimActionsInitiator>? {
  ...
  fun isPuttingLiteral(): Boolean {
    return !commandParts.isEmpty() && commandParts.last.action.id == "FimInsertCompletedLiteralAction"
  }
  ...
  fun buildCommand(): Command {
    if (commandParts.last.action.id == "FimInsertCompletedDigraphAction" || commandParts.last.action.id == "FimResetModeAction") {
  ...
  fun resetAll(commandPartNode: CommandPartNode<FimActionsInitiator>) {
  ...
  fun resetInProgressCommandPart(commandPartNode: CommandPartNode<FimActionsInitiator>) {
    count = 0
    setCurrentCommandPartNode(commandPartNode)
  }

  @TestOnly
  fun getCurrentTrie(): CommandPartNode<FimActionsInitiator> = currentCommandPartNode

  companion object {
    private val LOG = fimLogger<CommandBuilder>()
  }
}
