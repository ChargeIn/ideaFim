...
data class TernaryExpression(val condition: Expression, val then: Expression, val otherwise: Expression) : Expression() {
  ...
  override fun evaluate(editor: FimEditor, context: ExecutionContext, fimContext: FimLContext): FimDataType {
    return if (condition.evaluate(editor, context, fimContext).asDouble() != 0.0) {
      then.evaluate(editor, context, fimContext)
    } else {
      otherwise.evaluate(editor, context, fimContext)
    }
  }
}
