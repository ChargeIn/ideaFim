...
object UnequalsIgnoreCaseHandler : BinaryOperatorHandler() {

  override fun performOperation(left: FimDataType, right: FimDataType): FimDataType {
    return if (left is FimString && right is FimString) {
      FimInt(if (left.asString().compareTo(right.asString(), ignoreCase = true) != 0) 1 else 0)
    } else {
      FimInt(if (left.asDouble() != right.asDouble()) 1 else 0)
    }
  }
}
