...
data class DictionaryExpression(val dictionary: LinkedHashMap<Expression, Expression>) : Expression() {
  ...
  override fun evaluate(editor: FimEditor, context: ExecutionContext, fimContext: FimLContext): FimDataType {
    val dict = FimDictionary(linkedMapOf())
    for ((key, value) in dictionary) {
      val evaluatedVal = value.evaluate(editor, context, fimContext)
      var newFuncref = evaluatedVal
      if (evaluatedVal is FimFuncref && evaluatedVal.handler is DefinedFunctionHandler && !evaluatedVal.isSelfFixed) {
        newFuncref = evaluatedVal.copy()
        newFuncref.dictionary = dict
      }
      dict.dictionary[FimString(key.evaluate(editor, context, fimContext).asString())] = newFuncref
    }
    return dict
  }
}
