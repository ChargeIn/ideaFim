...
object MultiplicationHandler : BinaryOperatorHandler() {

  override fun performOperation(left: FimDataType, right: FimDataType): FimDataType {
    return if (left is FimFloat || right is FimFloat) {
      FimFloat(left.asDouble() * right.asDouble())
    } else {
      FimInt((left.asDouble() * right.asDouble()).toInt())
    }
  }
}
