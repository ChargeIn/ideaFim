...
import com.flop.idea.fim.fimscript.model.commands.mapping.MapCommand.SpecialArgument.EXPR
import com.flop.idea.fim.fimscript.model.commands.mapping.MapCommand.SpecialArgument.SCRIPT
...
data class MapCommand(val ranges: Ranges, val argument: String, val cmd: String) : Command.SingleExecution(ranges, argument) {
  ...
  @Throws(ExException::class)
  private fun executeCommand(editor: FimEditor?): Boolean {
    ...
    val mappingOwner =
      if (injector.fimscriptExecutor.executingFimscript) MappingOwner.IdeaFim.InitScript
      else MappingOwner.IdeaFim.Other
  ...
  companion object {
    ...
    private val logger = fimLogger<MapCommand>()
  }
  ...
  private fun parseCommandArguments(input: String): CommandArguments? {
    ...
    return fromKeys?.let {
      val toExpr = if (specialArguments.contains(EXPR)) {
        injector.fimscriptParser.parseExpression(toKeysBuilder.toString().trim()) ?: throw ExException("E15: Invalid expression: ${toKeysBuilder.toString().trim()}")
      } else {
