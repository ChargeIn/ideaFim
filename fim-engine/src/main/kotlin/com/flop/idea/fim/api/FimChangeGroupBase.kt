...
import com.flop.idea.fim.helper.inSingleCommandMode
import com.flop.idea.fim.helper.usesVirtualSpace
import com.flop.idea.fim.helper.fimStateMachine
import com.flop.idea.fim.listener.SelectionFimListenerSuppressor
import com.flop.idea.fim.mark.FimMarkConstants.MARK_CHANGE_END
import com.flop.idea.fim.mark.FimMarkConstants.MARK_CHANGE_POS
import com.flop.idea.fim.mark.FimMarkConstants.MARK_CHANGE_START
...
abstract class FimChangeGroupBase : FimChangeGroup {
  ...
  @JvmField
  protected var fimDocumentListener: ChangesListener? = null

  @JvmField
  protected var lastLower = true

  @JvmField
  protected var fimDocument: FimDocument? = null
   ...
   */
  override fun insertText(editor: FimEditor, caret: FimCaret, offset: Int, str: String) {
    (editor as MutableFimEditor).insertText(Offset(offset), str)
    caret.moveToInlayAwareOffset(offset + str.length)

    injector.markGroup.setMark(editor, MARK_CHANGE_POS, offset)
  }

  override fun insertText(editor: FimEditor, caret: FimCaret, str: String) {
    insertText(editor, caret, caret.offset.point, str)
  }

  open fun insertText(editor: FimEditor, caret: FimCaret, start: FimLogicalPosition, str: String) {
        ...
        for (lastStroke in myLastStrokes) {
          when (lastStroke) {
            ...
            is EditorActionHandlerBase -> {
              injector.actionExecutor.executeFimAction(editor, lastStroke, context, operatorArguments)
              strokes.add(lastStroke)
            }
  ...
  override fun repeatInsert(
    ...
    for (caret in editor.nativeCarets()) {
      if (repeatLines > 0) {
        ...
        val position = editor.logicalPositionToOffset(FimLogicalPosition(logicalLine, repeatColumn, false))
        for (i in 0 until repeatLines) {
          if (repeatAppend &&
            (repeatColumn < FimMotionGroupBase.LAST_COLUMN) &&
          ...
          if (repeatColumn >= FimMotionGroupBase.LAST_COLUMN) {
            ...
            val visualPosition = FimVisualPosition(visualLine + i, repeatColumn, false)
            val inlaysCount = injector.engineEditorHelper.amountOfInlaysBeforeVisualPosition(editor, visualPosition)
            caret.moveToVisualPosition(FimVisualPosition(visualLine + i, repeatColumn + inlaysCount, false))
            repeatInsertText(editor, context, updatedCount, operatorArguments)
          }
        }
  ...
  protected inner class FimChangesListener : ChangesListener {
    ...
    private fun getAdjustCaretActions(change: ChangesListener.Change): List<EditorActionHandlerBase> {
      ...
      if (oldOffset >= 0 && delta != 0) {
        ...
        val motionName = if (delta < 0) "FimMotionLeftAction" else "FimMotionRightAction"
        val action = injector.actionExecutor.findFimAction(motionName)!!
        val count = abs(delta)
        for (i in 0 until count) {
   ...
   * @param context The data context
   */
  override fun insertBeforeCursor(editor: FimEditor, context: ExecutionContext) {
    initInsert(editor, context, FimStateMachine.Mode.INSERT)
  }

  override fun insertAfterLineEnd(editor: FimEditor, context: ExecutionContext) {
    for (caret in editor.nativeCarets()) {
   ...
   * @param context The data context
   */
  override fun insertAfterCursor(editor: FimEditor, context: ExecutionContext) {
    for (caret in editor.nativeCarets()) {
   ...
   * @param context The data context
   */
  override fun insertLineStart(editor: FimEditor, context: ExecutionContext) {
    for (caret in editor.nativeCarets()) {
   ...
   * @param editor The editor to insert into
   */
  override fun insertBeforeFirstNonBlank(editor: FimEditor, context: ExecutionContext) {
    for (caret in editor.nativeCarets()) {
   ...
   */
  override fun initInsert(editor: FimEditor, context: ExecutionContext, mode: FimStateMachine.Mode) {
    val state = getInstance(editor)
    for (caret in editor.nativeCarets()) {
      caret.fimInsertStart = editor.createLiveMarker(caret.offset, caret.offset)
    ...
    if (cmd != null && state.isDotRepeatInProgress) {
      state.pushModes(mode, FimStateMachine.SubMode.NONE)
      if (mode === FimStateMachine.Mode.REPLACE) {
      ...
      strokes.clear()
      repeatCharsCount = 0
      val myFimDocument = fimDocument
      if (myFimDocument != null && fimDocumentListener != null) {
        myFimDocument.removeChangeListener(fimDocumentListener!!)
      }
      fimDocument = editor.document
      val myChangeListener = FimChangesListener()
      fimDocumentListener = myChangeListener
      fimDocument!!.addChangeListener(myChangeListener)
      oldOffset = editor.currentCaret().offset.point
      editor.insertMode = mode === FimStateMachine.Mode.INSERT
      state.pushModes(mode, FimStateMachine.SubMode.NONE)
    }
    notifyListeners(editor)
  }

  override fun runEnterAction(editor: FimEditor, context: ExecutionContext) {
    val state = getInstance(editor)
  ...
  override fun runEnterAboveAction(editor: FimEditor, context: ExecutionContext) {
    val state = getInstance(editor)
    if (!state.isDotRepeatInProgress) {
      // While repeating the enter action has been already executed because `initInsert` repeats the input
   ...
   */
  override fun processEscape(editor: FimEditor, context: ExecutionContext?, operatorArguments: OperatorArguments) {
    ...
    if (getInstance(editor).mode === FimStateMachine.Mode.REPLACE) {
      editor.insertMode = true
    }
    ...
    if (fimDocument != null && fimDocumentListener != null) {
      fimDocument!!.removeChangeListener(fimDocumentListener!!)
      fimDocumentListener = null
    }
    ...
    if (getInstance(editor).mode === FimStateMachine.Mode.INSERT) {
  ...
  private fun exitAllSingleCommandInsertModes(editor: FimEditor) {
    while (editor.inSingleCommandMode) {
      editor.fimStateMachine.popModes()
   ...
   * @param context The data context
   */
  override fun processEnter(editor: FimEditor, context: ExecutionContext) {
    if (editor.fimStateMachine.mode === FimStateMachine.Mode.REPLACE) {
      editor.insertMode = true
    }
  ...
  override fun processPostChangeModeSwitch(
    editor: FimEditor,
    context: ExecutionContext,
    toSwitch: FimStateMachine.Mode,
  ) {
    if (toSwitch === FimStateMachine.Mode.INSERT) {
      initInsert(editor, context, FimStateMachine.Mode.INSERT)
    }
  }
   ...
   */
  override fun processCommand(editor: FimEditor, cmd: Command) {
   ...
   */
  override fun processSingleCommand(editor: FimEditor) {
    getInstance(editor).pushModes(FimStateMachine.Mode.INSERT_NORMAL, FimStateMachine.SubMode.NONE)
    clearStrokes(editor)
  }
   ...
   *
   * @param editor The editor to delete from
   * @param caret  FimCaret on the position to start
   * @param count  The number of lines affected
   * @return true if able to delete the text, false if not
   */
  override fun deleteEndOfLine(editor: FimEditor, caret: FimCaret, count: Int, operatorArguments: OperatorArguments): Boolean {
   ...
   */
  override fun deleteLine(editor: FimEditor, caret: FimCaret, count: Int, operatorArguments: OperatorArguments): Boolean {
  ...
  override fun joinViaIdeaByCount(editor: FimEditor, context: ExecutionContext, count: Int): Boolean {
    val executions = if (count > 1) count - 1 else 1
    val allowedExecution = editor.nativeCarets().any { caret: FimCaret ->
      val lline = caret.getLogicalPosition().line
      val total = editor.lineCount()
      lline + count <= total
    }
  ...
  override fun joinViaIdeaBySelections(
    ...
    caretsAndSelections.forEach { (caret: FimCaret, range: FimSelection) ->
    ...
    editor.nativeCarets().forEach { caret: FimCaret ->
      caret.removeNativeSelection()
      val (line, column) = caret.getVisualPosition()
      if (line < 1) return@forEach
      val newVisualPosition = FimVisualPosition(line - 1, column, false)
  ...
  private fun removeLastNewLine(editor: FimEditor, range: TextRange, type: SelectionType?): Boolean {
   ...
   */
  override fun changeEndOfLine(editor: FimEditor, caret: FimCaret, count: Int, operatorArguments: OperatorArguments): Boolean {
   ...
   */
  override fun changeCharacters(editor: FimEditor, caret: FimCaret, operatorArguments: OperatorArguments): Boolean {
   ...
   */
  protected fun clearStrokes(editor: FimEditor) {
    strokes.clear()
    repeatCharsCount = 0
    for (caret in editor.nativeCarets()) {
      caret.fimInsertStart = editor.createLiveMarker(caret.offset, caret.offset)
  ...
  private fun activeTemplateWithLeftRightMotion(editor: FimEditor, keyStroke: KeyStroke): Boolean {
  ...
  companion object {
    ...
    private val logger = fimLogger<FimChangeGroupBase>()
     ...
     */
    fun getLinesCountInVisualBlock(editor: FimEditor, range: TextRange): Int {
