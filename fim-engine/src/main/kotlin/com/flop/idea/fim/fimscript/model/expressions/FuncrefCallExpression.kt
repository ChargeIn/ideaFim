...
data class FuncrefCallExpression(val expression: Expression, val args: List<Expression>) : Expression() {
  ...
  fun evaluateWithRange(ranges: Ranges?, editor: FimEditor, context: ExecutionContext, fimContext: FimLContext): FimDataType {
    val value = expression.evaluate(editor, context, fimContext)
    if (value is FimFuncref) {
      value.handler.ranges = ranges
      return value.execute(value.handler.name, args, editor, context, fimContext)
    } else {
  ...
  override fun evaluate(editor: FimEditor, context: ExecutionContext, fimContext: FimLContext): FimDataType {
    return evaluateWithRange(null, editor, context, fimContext)
  }
}
