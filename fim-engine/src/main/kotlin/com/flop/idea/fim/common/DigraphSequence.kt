...
import com.flop.idea.fim.common.DigraphResult.Companion.done
import com.flop.idea.fim.common.DigraphResult.Companion.handled
...
class DigraphSequence {
  ...
  fun processKey(key: KeyStroke, editor: FimEditor): DigraphResult {
  ...
  private fun specialKeyToKeyCode(key: KeyStroke): KeyStroke? {
    if (key.modifiers and KeyEvent.CTRL_DOWN_MASK != 0) {
      val specialKeyCode = injector.parser.parseFimScriptString("\\" + injector.parser.toKeyNotation(key))
  ...
  companion object {
    ...
    private val logger = fimLogger<DigraphSequence>()
  }
}
